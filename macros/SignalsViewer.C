...
Int_t colors[16] = {600, 632, 419, 1, 808, 616, 850, 591,  // colors to plot multiple signals on one canvas
...
Bool_t SignalsViewer(TString path = ".", Int_t ch = 0, 
                     Int_t thr = -164, Bool_t BL_flag = "kTRUE")
{
    //----- opening binary file 
...
*/
    ...
    channels[0].m = 0; channels[0].l = 0; channels[0].s = 0; channels[0].side = 'l'; 
    channels[1].m = 0; channels[1].l = 0; channels[1].s = 0; channels[1].side = 'r'; 
    channels[2].m = 0; channels[2].l = 0; channels[2].s = 1; channels[2].side = 'l'; 
    channels[3].m = 0; channels[3].l = 0; channels[3].s = 1; channels[3].side = 'r'; 
    channels[4].m = 0; channels[4].l = 0; channels[4].s = 2; channels[4].side = 'l'; 
    channels[5].m = 0; channels[5].l = 0; channels[5].s = 2; channels[5].side = 'r'; 
    channels[6].m = 0; channels[6].l = 0; channels[6].s = 3; channels[6].side = 'l'; 
    channels[7].m = 0; channels[7].l = 0; channels[7].s = 3; channels[7].side = 'r'; 
        ...
        for(Int_t j=0; j<tentries; j++)
        {
            ...
            if(hptr && sptr)
            {
                switch(cut)
                {
                    ...
                    case CutType::fAmpPE:   // amplitude & PE cut, provide amplitude min and max, PE min and max
                        if(sptr->GetAmplitude()>range[0] 
                           && sptr->GetAmplitude()<range[1]
                    ...
                    case CutType::fAmpPESig:   // amplitude, PE & base line sigma cut, provide amplitude min 
                                               // and max, PE min & max, base line sigma max
                        if(sptr->GetAmplitude()>range[0]
                           && sptr->GetAmplitude()<range[1]
                           && PE>range[2] && PE<range[3] 
                    ...
                    case CutType::fAmpPESigVeto:// amplitude, PE, base line sigma & veto cut, provide amplitude 
                                                // min and max, PE min & max, base line sigma max, veto is fixed
                                                // as false
                        if(sptr->GetAmplitude()>range[0] 
                           && sptr->GetAmplitude()<range[1]
                           && PE>range[2] && PE<range[3] 
