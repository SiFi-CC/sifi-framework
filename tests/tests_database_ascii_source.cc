...
protected:
    void SetUp() override
    {
        ...
        auto&& ascii_src = SIFI::make_ascii_source(tests_path + "params_ascii.txt");
        ascii_source = ascii_src.get();
        rdb()->addSource(std::move(ascii_src));
    ...
    SDatabase db;
    SIFI::SParAsciiSource* ascii_source{nullptr};
};
